...
export async function ensureInitialAdmin(environment: AuthEnv): Promise<void> {
  const localDefaults =
    environment.LOCAL_CONSOLE_DEFAULTS === true ||
    environment.LOCAL_CONSOLE_DEFAULTS === "true";
  const username = localDefaults
    ? "admin"
    : environment.INITIAL_ADMIN_USERNAME?.trim();
  const password = localDefaults
    ? "123456"
    : environment.INITIAL_ADMIN_PASSWORD;
  const apiKey = localDefaults
    ? "lynx_live_local_testing_only_1234567890"
    : environment.INITIAL_ADMIN_API_KEY?.trim();
...
export async function authenticateApiKey(
  ...
  const [user] = await database
    .select()
    .from(users)
    .where(eq(users.apiKeyHash, await sha256Hex(key)))
    .limit(1);
...
async function readSession(
  ...
  if (!(await constantTimeEqual(signature, await sign(secret, encoded))))
    return null;
  try {
